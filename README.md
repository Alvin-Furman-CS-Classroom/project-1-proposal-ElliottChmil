...
**Input:** Raw recipe dataset (CSV format) with columns: recipe_id, title, ingredients (comma-separated or JSON), instructions, prep_time, cuisine_type, course_type, difficulty, nutritional_info. May also include ingredient database (CSV) with: ingredient_name, category, allergen_info, substitution_options.
